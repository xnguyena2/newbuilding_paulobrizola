...
public class ControlEvent : MonoBehaviour ,IEventSystemHandler {
	...
	*/
			...
			if (namefloor == "block8_2") {
				...
				for (int i = 0; i < newLengthList2; i++) {
					if (i < list2.Length)
						...
						listpoint [i] = ratetio * changeStair [i - list2.Length] + orgPChange;
				}
				...
				for (int i = 0; i < newLengthList2; i++) {
					if (i < oldLength)
						...
						listpoint [i] = ratetio * changeStair2 [i - oldLength - changeStair1.Length] + orgPChange2;
				}
	...
	private IEnumerator officeClick(Button name){
		...
		showBlck (Block, 1);
		listNameCurrentBlock.Add ("block8_1");
		currentBlock = Block;
		currentFloor = Floor;		
	...
	public void bathRoomPress(){
		...
		int Floor = 1;//int.Parse (blcName [2].ToString ());
	...
	string buginfo = "building_Data\\data\\bug.txt";
